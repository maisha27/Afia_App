...
export function AdminSidebar({ active }: { active: AdminPage }) {
  return (
    <aside
      className="w-[224px] flex-shrink-0 flex flex-col"
      style={{ background: '#1F2A27', padding: '22px 16px' }}
    >
      {/* Logo + Admin badge */}
        ...
        {NAV.map(({ id, href, label, icon }) => {
          ...
          return (
            <Link
              ...
              style={
                isActive
                  ? { background: '#2C3A36', color: '#EAF3EF', fontWeight: 600 }
                  : { color: '#9DB3AC', fontWeight: 500 }
              }
            >
              {icon(isActive)}
              {label}
      ...
      <div
        className="mt-auto flex items-center gap-[10px] px-2 py-[10px]"
        style={{ borderTop: '1px solid #33413D' }}
      >
        <span
        ...
        <div className="flex-1 leading-[1.2]">
          <div className="text-[13px] font-semibold" style={{ color: '#DCE7E2' }}>
            Hasnain
          </div>
          <button
            type="button"
