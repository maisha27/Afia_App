...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const targets = (profilesRes.data ?? []).filter(
    (p) => (p.notification_prefs as Record<string, boolean> | null)?.daily === true,
  );
  ...
  for (const profile of targets) {
    ...
    if (!email) continue;
    try {
      await sendDailyNudge(email, profile.first_name ?? '');
      sent++;
    } catch (err) {
      failed++;
      console.error('[cron:daily-nudge] send failed:', { userId: profile.id, error: err });
    }
  }

  if (failed > 0 && failed >= Math.ceil(targets.length * 0.1)) {
    captureMessage(`[cron:daily-nudge] high failure rate: ${failed}/${targets.length} emails failed`, 'error');
  }
